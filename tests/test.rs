...
mod tests {
    ...
    fn test_get_app_env() {
        ...
        assert_eq!(
            result, "development",
            "{} does not match {:?}", icarus_envy::keys::APP_ENV,
            result
        )
    }
    ...
    fn test_get_backend_port() {
        ...
        assert_eq!(
            result, "8001",
            "{} does not match {:?}", icarus_envy::keys::BACKEND_PORT,
            result
        )
    }
    #[test]
    fn test_get_frontend_url() {
        ...
        assert_eq!(
            ...
            "{} does not match {:?}", icarus_envy::keys::FRONTEND_URL,
            result
        )
    }
    #[test]
    fn test_get_rust_log() {
        ...
        assert_eq!(
            result, "debug",
            "{} does not match {:?}", icarus_envy::keys::RUST_LOG,
            result
        )
    }
    #[test]
    fn test_get_allowed_origins() {
        ...
        assert_eq!(
            result, "https://soaricarus.com,https://www.soaricarus.com",
            "{} does not match {:?}", icarus_envy::keys::ALLOWED_ORIGINS,
            result
        )
    }
